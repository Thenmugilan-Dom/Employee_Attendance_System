  ...
  static async checkin(employeeId) {
    try {
      ...
      const [existing] = await db.execute(
        'SELECT id FROM attendance WHERE userId = ? AND DATE(checkInTime) = ?',
        [userId, today]
      );
      ...
      const [result] = await db.execute(query, [userId, checkInTime, today]);
  ...
  static async checkout(employeeId) {
    try {
      ...
      const query = `
        UPDATE attendance 
        SET checkOutTime = ?, 
            totalHours = TIMESTAMPDIFF(HOUR, checkInTime, ?)
        WHERE userId = ? AND DATE(checkInTime) = ? AND checkOutTime IS NULL
      `;
      
      const checkOutTime = new Date();
      const [result] = await db.execute(query, [checkOutTime, checkOutTime, userId, today]);
      
      if (result.affectedRows > 0) {
        const [record] = await db.execute(
          'SELECT * FROM attendance WHERE userId = ? AND DATE(checkInTime) = ?',
          [userId, today]
        );
